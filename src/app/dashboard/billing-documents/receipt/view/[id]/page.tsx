...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return { title: document?.docNo ?? "ใบเสร็จรับเงิน" };
}
...
export default async function ViewReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <PrintBillingDocumentView
      document={document}
      editHref={allowEdit ? `/dashboard/billing-documents/receipt/edit/${id}` : undefined}
      closeHref="/dashboard/billing-documents/receipt"
    />
  );
}
