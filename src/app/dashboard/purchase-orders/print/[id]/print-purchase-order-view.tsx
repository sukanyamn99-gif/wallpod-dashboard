...
// below), the same convention already used by the billing-documents print
// view for this kind of document.
function DocumentBody({ order, copyLabel }: { order: PurchaseOrder; copyLabel: string }) {
  ...
  return (
    // 250mm, not 297mm/277mm — the PDF download route (/api/print-pdf)
    // renders A4 with 20mm+15mm top/bottom margins, leaving only 262mm of
    // usable page height; a short PO (few items) needs the min-height to
    // stay safely under that so the signature block doesn't get pushed
    // onto a second, otherwise-blank page.
      ...
      <table className="mt-2 w-full table-fixed border-collapse border border-black text-center">
        ...
        <tbody>
          {rows.map((item, i) => (
            ...
            // horizontal lines left, per feedback that the row-by-row
            // lines made the table look too busy.
            <tr key={item.id} className="h-8">
              ...
              <td className="border-r border-black p-1">{item.productName ? "7%" : ""}</td>
              <td className="border-r border-black p-1">
  ...
  return (
    <div className="mx-auto max-w-3xl bg-white p-6 text-black print:p-0">
      ...
      {/* Printed as two consecutive pages: ต้นฉบับ (original) for the
          supplier, สำเนา (copy) kept on file — standard practice for this
          kind of document, matching the billing-documents print view.
          break-after-page only affects print output; on screen both
          copies simply stack. */}
      <div className="break-after-page">
        <DocumentBody order={order} copyLabel="ต้นฉบับ" />
      </div>
      <DocumentBody order={order} copyLabel="สำเนา" />
    </div>
  );
}
