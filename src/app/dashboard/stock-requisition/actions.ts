...
async function generateDocNo(supabase: Awaited<ReturnType<typeof createClient>>): Promise<string> {
  ...
  // a deleted requisition leaves a gap, and count+1 would then collide
  ...
  const { data, error } = await supabase.from("stock_requisitions").select("doc_no").like("doc_no", `${prefix}%`);
