...
-- ============ Finished Goods (separate from raw-material stock_products) ============
-- A fully separate system: finished goods produced from a JOB, received in
-- with a free-typed quantity/price, and withdrawn automatically when a
-- ใบกำกับภาษี (tax_invoice) is issued — kept structurally apart from
-- raw-material stock so a JOB's linked requisition cost (see
-- getJobLinkedCostSummary) can never mix the two.
...
-- ============ เอกสารซื้อ: ใบขอซื้อ (PR) → ใบสั่งซื้อ (PO) → ใบรับสินค้า ============
-- ใบรับสินค้า here is a new document tied specifically to a purchase order
-- (tracks received vs. ordered per line) — separate from the existing
-- goods_receipts/"รับเข้าสินค้า" ad-hoc restocking feature above. Reuses the
